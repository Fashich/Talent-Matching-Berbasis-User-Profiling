      ...
      <div className="space-y-3">
        {list.length === 0 ? (
          ...
          list.map((j) => {
            ...
            return (
              <div key={j.id} className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800 p-5">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-sm font-semibold text-gray-800 dark:text-slate-100">{j.tanggal}</p>
                    {!isSiswa && <p className="text-xs text-gray-400 dark:text-slate-500">{s?.nama} · {s?.kelas}</p>}
                  </div>
