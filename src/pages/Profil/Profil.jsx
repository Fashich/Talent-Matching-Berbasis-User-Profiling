...
const Profil = () => {
  ...
  if (!data) {
    return <div className="max-w-3xl mx-auto text-sm text-gray-500">Data siswa tidak ditemukan.</div>;
  }
    ...
    <div className="max-w-3xl mx-auto space-y-4 text-gray-900">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="font-semibold text-gray-800">{data.nama}</h3>
            <p className="text-xs text-gray-400">NISN {data.nisn} · {data.kelas} · Jurusan {data.jurusan}</p>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
          <div>
            ...
            <input value={form.pendidikan} onChange={set('pendidikan')} placeholder="mis. SMK Kelas XI — Rekayasa Perangkat Lunak" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Preferensi (lokasi, jenis pekerjaan, dsb.)</label>
            <input value={form.preferensi} onChange={set('preferensi')} placeholder="mis. Lokasi Surabaya, jenis pekerjaan Web Developer" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Portofolio (opsional, tautan)</label>
            <input value={form.portofolio} onChange={set('portofolio')} placeholder="https://..." className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {form.minat.map((m, i) => (
              <span key={i} className="flex items-center gap-1.5 bg-blue-50 text-blue-700 border border-blue-100 px-3 py-1 rounded-full text-xs font-medium">
                {m}
                <button type="button" onClick={() => removeMinat(i)} className="text-blue-400 hover:text-blue-700">
                  <Trash2 size={12} />
                </button>
              </span>
            ))}
            {form.minat.length === 0 && <p className="text-xs text-gray-400">Belum ada minat ditambahkan.</p>}
          </div>
          <div className="flex gap-2">
            ...
            />
            <button type="button" onClick={addMinat} className="px-3 py-2 rounded-lg text-sm font-medium text-blue-600 border border-blue-200 hover:bg-blue-50 transition">
              Tambah
            </button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <button type="button" onClick={addPengalaman} className="flex items-center gap-1.5 text-xs font-medium text-blue-600 hover:text-blue-700 px-3 py-1.5 rounded-lg border border-blue-100 hover:bg-blue-50 transition">
              <Plus size={12} />
              Tambah Pengalaman
            </button>
          </div>
          {form.pengalaman.length === 0 && <p className="text-xs text-gray-400">Belum ada pengalaman ditambahkan.</p>}
          <div className="space-y-3">
            {form.pengalaman.map((p, i) => (
              <div key={i} className="border border-gray-100 rounded-lg p-3 relative">
                <button type="button" onClick={() => removePengalaman(i)} className="absolute top-2 right-2 text-gray-300 hover:text-red-600">
                  <Trash2 size={14} />
                </button>
                <input
                  value={p.judul}
                  ...
                  className="w-full px-3 py-1.5 border border-gray-200 rounded-lg text-sm font-medium mb-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
                <textarea
                  value={p.deskripsi}
